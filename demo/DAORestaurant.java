...
public class DAORestaurant {
    ...
    private List<Categorie> getCatMemo(String cat) throws SQLException {
        PreparedStatement getCat = connection.prepareStatement(
                "SELECT * FROM Categories JOIN APourCatParent ON APourCatParent.nomCatParent = ? JOIN APourCatParent ON APourCatParent.nomCatFils = Categories.nomCategorie");
        ...
        while (rSet.next()) {
            ...
            if (!catList.contains(catFille))
                catList.add(catFille);
    ...
    public void getCategories(String cat) throws SQLException {
        ...
        for (Categorie categorie : catList) {
            PreparedStatement getMailRest = connection
                    .prepareStatement("SELECT mailR FROM FaitPartieCat WHERE nomCategorie LIKE ?");
            ...
            while (rSet.next()) {
                ...
                if (restListCat.contains(restaurant))
                    restListCat.add(restaurant);
